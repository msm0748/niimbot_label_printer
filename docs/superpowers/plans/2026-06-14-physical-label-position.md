...
**Architecture:** Extend `LabelText` with a public physical-position enum. The renderer measures the laid-out text block and chooses its paint offset on the normal canvas; for rotated labels it maps physical horizontal placement to the pre-rotation vertical axis. The probe app exposes the option and passes it through its existing document builder.
...
Expected: FAIL because all rotated outputs remain vertically centered before
rotation.

- [ ] **Step 7: Map placement to the pre-rotation vertical axis**

Pass the document orientation into `_paintText`. For rotated output, calculate
the y offset so the clockwise-rotated text block lands at the requested physical
horizontal position:
